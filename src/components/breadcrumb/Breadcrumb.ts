...
export class Breadcrumb implements m.Component<IBreadcrumbAttrs> {
  public view({ attrs, children }: m.Vnode<IBreadcrumbAttrs>) {
    const { class: className, separator = '/', size, ...htmlAttrs } = attrs;
  ...
  private renderChildren(children: m.ChildArray, { separator }: IBreadcrumbAttrs) {
    return children
      .filter((item) => item != null)
      .map((item) => [
        item,
        m(`span.${Classes.BREADCRUMB_SEPARATOR}`, separator)
      ]);
  }
}
